...
def adjust_per(users_dict, sub_users_dict, X, Y):
    ...
    for u in users_dict.keys():
        if u in sub_users_dict.keys():
            users_dict[u] = users_dict[u]*X/100 + \
                sub_users_dict[u]/(nav_after - realtime_nav)*Y
        else:
            users_dict[u] = users_dict[u]*X/100 + \
                0/(nav_after - realtime_nav)*Y
    return users_dict


if __name__ == '__main__':
    global base_path, data_path, output_path
    Basic_nav = 795.0 * 1000000
    Future_nav = 0.0 * 1000000
    ...
    for idx, date_ in enumerate(date_list):
        ...
        for u in sub_data_df['ClientID'].tolist():
            sub_users_dict[u] = sub_data_df[sub_data_df['ClientID']
                                            == u]['Monney'].sum()
    ...
    # print output
    for u in users_dict.keys():
        print('{} -- {}% -- {} -- {} -- {}'.format(u, round(users_dict[u], 2), data_df[data_df['ClientID'] == u]['Monney'].sum(), round(
            users_dict[u]*TODAY_NAV/100), round(users_dict[u]*TODAY_NAV/100)-data_df[data_df['ClientID'] == u]['Monney'].sum()))
